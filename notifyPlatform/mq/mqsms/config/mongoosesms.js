...
mongoose.connection.once('open', () => { //we check the mongodb connection
    console.log(process.env.GREEN_COLOR, "Connected to DB Server : " + process.env.MONGODBSMS_URI);
});
...
mongoose.connection.on('error', (error) => {  //we need to know if connection works, particularly at the start if we didn't connect with it.
    ...
    console.log(process.env.RED_COLOR, "FATAL ERROR : failed to connect to db server : " + process.env.MONGODBSMS_URI);
...
const initializeMongooseConnection = async () => {
    ...
    await mongoose.connect(process.env.MONGODBSMS_URI, { useNewUrlParser: true, useFindAndModify: false, useCreateIndex: true });
