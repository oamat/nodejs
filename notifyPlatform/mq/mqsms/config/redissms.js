...
rclient.on('connect', function () { //we check the redis connection
    console.log(process.env.GREEN_COLOR, logTime(new Date()) + 'Connected to Redis Server: ' + process.env.REDISSMS_IP + ":" + process.env.REDISSMS_PORT);
});

rclient.on("error", function (error) {  //we check the redis connection is fine
    let date = new Date();
    console.log(process.env.RED_COLOR, logTime(date) + " FATAL ERROR : failed to connect to Redis SMS server : " + process.env.REDISSMS_IP + ":" + process.env.REDISSMS_PORT);
