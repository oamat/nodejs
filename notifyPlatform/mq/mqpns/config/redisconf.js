...
rclient.on('connect', function () { //we check the redis connection
    console.log(process.env.GREEN_COLOR, logTime(new Date()) + 'Connected to Redis Server: ' + process.env.REDISCONF_IP + ":" + process.env.REDISCONF_PORT);
});

rclient.on("error", function (error) {  //we check the redis connection is fine
    let date = new Date();
    console.log(process.env.RED_COLOR, logTime(date) + " FATAL ERROR : failed to connect to Redis CONF server : " + process.env.REDISCONF_IP + ":" + process.env.REDISCONF_PORT);
