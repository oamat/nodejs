...
rclient.on('connect', function () { //we check the redis connection
    console.log(process.env.GREEN_COLOR, 'Connected to Redis Server: ' + process.env.REDISPNS_IP + ":" + process.env.REDISPNS_PORT);
});

rclient.on("error", function (error) {  //we check the redis connection is fine
    console.log(process.env.RED_COLOR, "FATAL ERROR : failed to connect to Redis server : " + process.env.REDISPNS_IP + ":" + process.env.REDISPNS_PORT);
