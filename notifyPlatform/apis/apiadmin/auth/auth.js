...
const { hmget } = require('../util/redisconf');
...
const auth = async (req, res, next) => {    
    try {
        if (!req || !req.body || !req.body.contract) {  //first we check the body params request. 
            ...
            const contractConf = await hmget(["contractadmin:" + decoded.contract, "jwt", "frontend"]); //this method is Async, but we can get in parallel until need it (with await 3 steps after). 
            if (decoded.contract != req.body.contract) { //check that contract in request is the same than contract in jwt
                ...
                if (token != contractConf[0]) { //check that jwt was created from this server and exist in redis Conf contract, we need to wait the result.                         
