...
rclient.on("error", function (error) {  //we check the redis connection is fine
    let date = new Date();
    console.log(process.env.RED_COLOR, logTime(date) + " FATAL ERROR : failed to reconnect to Redis CONF server : " + process.env.REDISCONF_IP + ":" + process.env.REDISCONF_PORT);
