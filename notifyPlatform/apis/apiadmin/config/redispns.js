...
rclient.on('connect', function () { //we check the redis connection
    console.log(process.env.GREEN_COLOR, logTime(new Date()) + 'Connected to Redis BUS-PNS Server: ' + process.env.REDISPNS_IP + ":" + process.env.REDISPNS_PORT);
});

rclient.on("error", function (error) {  //we check the redis connection is fine
    let date = new Date();
    console.log(process.env.RED_COLOR, logTime(date) + " FATAL ERROR : failed to reconnect to Redis BUS-PNS server : " + process.env.REDISPNS_IP + ":" + process.env.REDISPNS_PORT);
