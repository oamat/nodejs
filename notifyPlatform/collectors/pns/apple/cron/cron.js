...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        console.log(process.env.GREEN_COLOR, logTime(new Date()) + "initializing APPLE cron at " + dateFormat(new Date()) + " with interval : " + interval);
        if (cron) {
...
const stopCron = async () => { //stop cron only when cron is switched on
    try {
        if (cron) {
            console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stoping Apple cron at " + dateFormat(new Date()));
            clearInterval(cron);
            cron = null;
        }
    } catch (error) {
...
const sendNextPNS = async () => {
    try {
        ...
        if (pnsJSON) {
            ...
            if ((pns.expire) && (Date.now() > pns.expire)) { // treat the expired PNS
                pns.expired = true;
                pns.status = 4;
...
const checksController = async () => {
    ...
    if (cronChanged) { //some param changed in cron, we need to restart or stopped.
        if (cronStatus) { //cron must to be started                       
            console.log(process.env.GREEN_COLOR, logTime(new Date()) + "Re-Start Apple cron...");
            cronChanged = false;
            await stopCron();
