    ...
    try {
        ...
        if (pnsJSON) {
            ...
            if ((pns.expire) && (date > pns.expire)) { // is the PNS expired?
                ...
                updatePNS(pns).catch(error => { console.log(process.env.YELLOW_COLOR, logTime(date) + error.message) }); //update PNS in MongoDB, is the last task, it's unnecessary await
                ...
                Promise.all([ //Always we need delete ID in PNS_IDS SET, in error case we continue
                    updatePNS(pns).catch(error => { console.log(process.env.YELLOW_COLOR, logTime(date) + error.message); }), // update PNS in MongoDB, in error case we continue
                    sismember(PNS_IDS, pns._id).catch(error => { console.log(process.env.YELLOW_COLOR, logTime(date) + error.message); }), //delete from redis ID control, in error case we continue
                    sendPNS(pns).catch(error => { console.log(process.env.YELLOW_COLOR, logTime(date) + error.message); return 3; }) // send PNS to operator, //return status: 0:notSent, 1:Sent, 2:Confirmed, 3:Error, 4:Expired 
...
const startController = async (intervalControl) => {
    try {
        ...
        console.log(process.env.RED_COLOR, logTime(new Date()) + "ERROR: we cannot start APPLE cron Controller. . Process continuing... " + error.message);
        //console.error(error); //continue the execution cron
    }
}
