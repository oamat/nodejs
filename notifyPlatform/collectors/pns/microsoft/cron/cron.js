...
const startController = async (intervalControl) => {
    try {
        console.log(process.env.GREEN_COLOR, logTime(new Date()) + "initializing MICROSOFT cronController at " + dateFormat(new Date()) + " with intervalControl : " + intervalControl);
    ...
    try {
        ...
        Promise.all([ //In error case we continue with other tasks
            ...
            if (cronControllerChanged) {
                console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stopping and Re-starting MICROSOFT cronController at " + dateFormat(new Date()));
