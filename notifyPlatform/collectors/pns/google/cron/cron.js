...
//Variables
const defaultOperator = "AND"; //default operator for this collector: "AND"
const defaultCollector = "collectorpns:" + defaultOperator; // default collector, for configurations
var operator = defaultOperator; //default operator for this collector: "AND"
...
const sendNextPNS = async () => {
    try {
        const pnsJSON = await nextPNS(); //get message with rpop command from PNS.AND.1, 2, 3 
        if (pnsJSON) {
...
const nextPNS = async () => {
    try {
        return await rpop(channels.channel0) || await rpop(channels.channel1) || await rpop(channels.channel2) || await rpop(channels.channel3); //return the next pns by priority order.  
    } catch (error) {
