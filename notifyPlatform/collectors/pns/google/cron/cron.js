...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        console.log(process.env.GREEN_COLOR, logTime(new Date()) + "initializing GOOGLE cron at " + dateFormat(new Date()) + " with interval : " + interval);
        if (cron) {
            console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "GOOGLE cron is executing, so we don't need re-start it.");
        } else {
        ...
        console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "ERROR: we cannot start GOOGLE cron . Process continuing... " + error.message);
...
const stopCron = async () => { //stop cron only when cron is switched on
    try {
        if (cron) {
            console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stoping Google cron at " + dateFormat(new Date()));
            clearInterval(cron);
            cron = null;
        }
    } catch (error) {
...
const checksController = async () => {
    ...
    if (cronChanged) { //some param changed in cron, we need to restart or stopped.
        if (cronStatus) { //cron must to be started                       
            console.log(process.env.GREEN_COLOR, logTime(new Date()) + "Re-Start Google cron...");
            cronChanged = false;
            await stopCron();
