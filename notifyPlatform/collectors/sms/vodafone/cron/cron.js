...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        ...
        console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "ERROR: we cannot start VODAFONE cron . Process continuing... " + error.message);
...
const stopCron = async () => { //stop cron only when cron is switched on
    try {
        if (cron) {
            console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stoping Vodafone cron at " + dateFormat(new Date()));
            clearInterval(cron);
            cron = null;
        }
    } catch (error) {
