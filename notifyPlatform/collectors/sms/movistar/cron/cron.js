...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        console.log(process.env.GREEN_COLOR, logTime(new Date()) + "initializing MOVISTAR cronMain at  with interval : " + interval);
        if (cron) {
...
const stopCron = async () => { //stop cron only when cron is switched on
    try {
        if (cron) {
            console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stoping MOVISTAR cronMain.");
            clearInterval(cron);
            cron = null;
        }
    } catch (error) {
...
const sendNextSMS = async () => {
    try {
        ...
        if (smsJSON) {
            let date = new Date();
            const sms = new Sms(JSON.parse(smsJSON)); // convert json text to json object               
            // if ((sms.expire) && (date > sms.expire)) { // is the SMS expired?  //EXPIRED         
...
const startController = async (intervalControl) => {
    try {
        console.log(process.env.GREEN_COLOR, logTime(new Date()) + "initializing MOVISTAR cronController with intervalControl : " + intervalControl);
        ...
        console.log(process.env.RED_COLOR, logTime(new Date()) + "ERROR: we cannot start MOVISTAR cron Controller. . Process continuing... " + error.message);
        //console.error(error); //continue the execution cron
    }
}
        ...
        if (cronConf && Object.keys(cronConf).length > 1) {
            Promise.all([ //In error case we continue with other tasks                
                ...
                if (cronControllerChanged) {
                    console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "Stopping and Re-starting MOVISTAR cronController ");
                ...
                console.log(process.env.GREEN_COLOR, logTime(new Date()) + "MOVISTAR cronMain interval [" + interval + "ms], cronController interval : [" + intervalControl + "ms], collector operator  [" + operator + "] and status [" + cronStatus + "](1:ON, 0:OFF).");
