...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        ...
        console.log(process.env.RED_COLOR, logTime(new Date()) + "ERROR: we cannot start MOVISTARVIP cron." + error.message);
...
const checkOperator = async () => { //change operator for HA
    try {
        let newOperator = await hgetConf(defaultCollector, "operator");  //"MOV", "VIP", "VOD", "ORA",... //change operator for HA
