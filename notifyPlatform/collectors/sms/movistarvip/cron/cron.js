    ...
    try {
        ...
        if (smsJSON) {
            ...
            if ((sms.expire) && (Date.now() > sms.expire)) { // is the SMS expired?
                ...
                if (operator == defaultOperator) { //VIPISTAR-VIP WILL SEND //If we change operator for contingency we change sms to other list
...
const checksController = async () => {
    try {
        ...
        Promise.all([ //In error case we continue with other tasks
            ...
            console.log(process.env.GREEN_COLOR, logTime(new Date()) + "MOVISTAR-VIP cronController : cronMain intervalControl is " + interval + ", cronController interval is " + interval + ", operator is '" + operator + "' and status is " + cronStatus + " ([1:ON, 0:OFF]).");
