                ...
                if (require('path').extname(filename) == ".json") {
                    ...
                    notifications.forEach(async (pnsJSON, index) => {
                        try {
                            ...
                            if (!tokenConf || !tokenConf.token || !tokenConf.operator) throw new Error("This uuiddevice is not register, we cannot find its token neither operator : 'tokenpns" + pns.application + ":" + pns.uuiddevice) //0:notSent, 1:Sent, 2:Confirmed, 3:retry, 4:Expired, 5:Error, 5:token not found (not register)
