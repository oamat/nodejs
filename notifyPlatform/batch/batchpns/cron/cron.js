...
const startCron = async (interval) => { //Start cron only when cron is stopped.
    try {
        ...
        console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "ERROR: we cannot start batchPNS . Process continuing... " + error.message);
...
const stopCron = async () => { //stop cron only when cron is switched on
    try {
        ...
        console.log(process.env.YELLOW_COLOR, logTime(new Date()) + "ERROR: we cannot stop batchPNS. Process continuing... " + error.message);
        //console.error(error); //continue the execution cron
    }
}
...
const checksController = async () => {
    ...
    if (cronChanged) { //some param changed in cron, we need to restart or stopped.
        if (cronStatus) { //cron must to be started                       
            console.log(process.env.GREEN_COLOR, logTime(new Date()) + "Re-Start batchPNS cron..." + cronStatus);
            cronChanged = false;
            await stopCron();
