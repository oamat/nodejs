...
const init = async function () {
    ...
    /* await getTestAll();
    let finalArray = [];
    let iterator = arrayOfArrays.values();
...
const recursiveMethod = async function (arrayComponents) {
    let iterator = arrayComponents.values();
    for (let component of iterator) {
        let nextArrayComponents = await getAllComponents(PREFIX + component);  //console.log (arrayComponents2);
        arrayOfArrays.push(nextArrayComponents); 
        if (nextArrayComponents.length > 0) {
            recursiveMethod (nextArrayComponents);
        }
    }
}
...
const getTableLigth = async function () {
    let arrayComponents0 = [];
    let arrayComponents1 = [];
    let arrayComponents2 = [];
    ...
    arrayOfArrays.push (arrayComponents2, arrayComponents1, arrayComponents0, arrayComponent );    
}
